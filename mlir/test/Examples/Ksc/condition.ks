...
; MLIR: %c10{{.*}} = constant 10 : i64
; MLIR: %c20{{.*}} = constant 20 : i64
; MLIR: %[[eq1:[0-9]+]] = call @eq(%c10{{.*}}, %c20{{.*}}) : (i64, i64) -> i1
; MLIR: %[[sel1:[0-9]+]] = select %[[eq1]], %[[foo1]], %[[bar1]] : i64

; LLVM: %[[foo1:[0-9]+]] = call i64 @foo(i64 30)
; LLVM: %[[bar1:[0-9]+]] = call i64 @bar(i64 40)
; LLVM: %[[eq1:[0-9]+]] = call i1 @eq(i64 10, i64 20)
; LLVM: %[[sel1:[0-9]+]] = select i1 %[[eq1]], i64 %[[foo1]], i64 %[[bar1]]
...
; MLIR: %[[eq2:[0-9]+]] = call @eq(%[[foo_cond]], %c60{{.*}}) : (i64, i64) -> i1
...
; LLVM: %[[eq2:[0-9]+]] = call i1 @eq(i64 %[[foo_cond]], i64 60)
